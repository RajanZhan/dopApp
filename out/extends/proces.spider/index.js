...
if (cluster.isMaster) {
    ...
    async function main() {
        try {
            var workerInitFinishedCount = 0;// 记录完成初始化的子进程数
                ...
                if (msg) {
                    switch (msg.type) {
                        case "toMaster.callModelCb":
                            ...
                            break;
                        case "toMaster.workerInitFinished":
                        workerInitFinishedCount++;
                        break;
                    }
                }
            });
            ...
            while(workerInitFinishedCount < Number($config.workers))
            {
                await utils.sleep(500);
                //console.log("等待子进程初始化完成");
            }
           
            //await utils.callToWorker("sys@workerInit")
            await mainfunc();
