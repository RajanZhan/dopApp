...
class c extends baseModel{
    ...
    async addLog(data,t){
        try{
            if((!data.userId) || (!data.type) || (!data.money) || (!data.direction)) 
            {
                console.log(data);
                throw new Error("资金变动日志 数据不完整");
            }

            var res = null;
            if(t)
            {
                res = await this.dataModels.balanceChangeLog.create(data,{transaction:t});
            }
            else
            {
                res = await this.dataModels.balanceChangeLog.create(data);
            }
    ...
    async delArticle(id){
        try{
            ...
            let cacheKey = this.getGoodsCacheKey(id);
